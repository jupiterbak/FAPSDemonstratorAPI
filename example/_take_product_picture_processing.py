...
def periodic_main():
    ...
    connection, channel, exchange, queue = connect(
        ...
        'esys',
        'esys',
        "FAPS_DEMONSTRATOR_ImageProcessing_ProcessingSignals",
        "FAPS_DEMONSTRATOR_ImageProcessing_ProcessingSignals"
    )
    ...
    channel.basic_publish(exchange="FAPS_DEMONSTRATOR_ImageProcessing_ProcessingSignals",
                               routing_key='',
    ...
    if demonstrator_program.connect(): ## Demonstrator is connected
        ...
        threading.Timer(INTERVAL, periodic_main).start()
    else:
