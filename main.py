...
class Demo:
    ...
    def generateProgram(self,programObject):
        programObject.reset()
        # ===========================================================================================================================
        ...
        for i in range(16):
            ...
            # Move to Origin
            if(i== 0):
                programObject.append_instruction(Command.CMD_POS_BASIC_SEGMENT, CommandMode.WCD, PT_CURRENT, PT_PALLETE_0_START + self.fromPositions[i],0,ParameterMode.ABSOLUTE,0)
                last_position = PT_PALLETE_0_START + self.fromPositions[i]
            else:
                programObject.append_instruction(Command.CMD_POS_BASIC_SEGMENT, CommandMode.WCD, last_position, PT_PALLETE_0_START + self.fromPositions[i],0,ParameterMode.ABSOLUTE,0)
                last_position = PT_PALLETE_0_START + self.fromPositions[i]
            ...
            programObject.append_instruction(Command.CMD_WAIT_TIME, CommandMode.WCD, 0,0,0,ParameterMode.ABSOLUTE,200)
            # Move to Target Pos
            programObject.append_instruction(Command.CMD_POS_BASIC_SEGMENT, CommandMode.WCD, last_position, PT_PALLETE_1_START + self.toPositions[i],0,ParameterMode.ABSOLUTE,0)
            ...
            if (not (demonstrator_is_running is None)) \
                    ...
                    and (demonstrator_product_ready != 0):                        
                        ...
                        if(delta.total_seconds() > 20):
                            myprogram = self.generateProgram(self.demonstrator_program)
                            myprogram.execute()
                            self.start = datetime.now()
