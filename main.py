...
if __name__ == '__main__':
    ...
    fromPositions = list(range(0, 16))
    toPositions = list(range(0, 16))

    def generateProgram(programObject):
        programObject.reset()
        # ===========================================================================================================================
        programObject.append_instruction(Command.CMD_SET_PATH_VELO,
                                        CommandMode.WCD,
                                        10,0,0,ParameterMode.ABSOLUTE,0)
        # ===========================================================================================================================
        #shuffle(fromPositions)
        #shuffle(toPositions)
        last_position = 0
        # Main For Loop
        for i in range(16):
            # Wait for 200 ms
            programObject.append_instruction(Command.CMD_WAIT_TIME, CommandMode.WCD, 0,0,0,ParameterMode.ABSOLUTE,200)
            # Open the gripper
            programObject.append_instruction(Command.CMD_SET_CONDITION, CommandMode.WCD, CND_GRIPPER_OPEN,1,0,ParameterMode.ABSOLUTE,0)
            # Move to Origin
            if(i== 0):
                programObject.append_instruction(Command.CMD_POS_BASIC_SEGMENT, CommandMode.WCD, PT_CURRENT, PT_PALLETE_0_START + fromPositions[i],0,ParameterMode.ABSOLUTE,0)
                last_position = PT_PALLETE_0_START + fromPositions[i]
            else:
                programObject.append_instruction(Command.CMD_POS_BASIC_SEGMENT, CommandMode.WCD, last_position, PT_PALLETE_0_START + fromPositions[i],0,ParameterMode.ABSOLUTE,0)
                last_position = PT_PALLETE_0_START + fromPositions[i]
            # Wait until movement is finished
            programObject.append_instruction(Command.CMD_WAIT_POS_BASIC_SEG_END, CommandMode.WCD, 0,0,0,ParameterMode.ABSOLUTE,0)            
            # Close the gripper
            programObject.append_instruction(Command.CMD_SET_CONDITION, CommandMode.WCD, CND_GRIPPER_OPEN,1,0,ParameterMode.ABSOLUTE,0)
            # Wait for 200 ms
            programObject.append_instruction(Command.CMD_WAIT_TIME, CommandMode.WCD, 0,0,0,ParameterMode.ABSOLUTE,200)
            # Move to Target Pos
            programObject.append_instruction(Command.CMD_POS_BASIC_SEGMENT, CommandMode.WCD, last_position, PT_PALLETE_1_START + toPositions[i],0,ParameterMode.ABSOLUTE,0)
            last_position = PT_PALLETE_1_START + toPositions[i]
            # Wait until movement is finished
            programObject.append_instruction(Command.CMD_WAIT_POS_BASIC_SEG_END, CommandMode.WCD, 0,0,0,ParameterMode.ABSOLUTE,0)  
            # Wait for 200 ms
            programObject.append_instruction(Command.CMD_WAIT_TIME, CommandMode.WCD, 0,0,0,ParameterMode.ABSOLUTE,200)
        ...
        programObject.append_instruction(Command.CMD_SET_CONDITION, CommandMode.WCD, CND_PRODUCT_ACTION,1,0,ParameterMode.ABSOLUTE,0)
        # Wait for 1000 ms
        programObject.append_instruction(Command.CMD_WAIT_TIME, CommandMode.WCD, 0,0,0,ParameterMode.ABSOLUTE,1000)
        # Goto to camera position
        programObject.append_instruction(Command.CMD_POS_BASIC_SEGMENT, CommandMode.WCD, last_position, PT_PRODUCT_CAMERA,0,ParameterMode.ABSOLUTE,0)
        # Wait until movement is finished
        programObject.append_instruction(Command.CMD_WAIT_POS_BASIC_SEG_END, CommandMode.WCD, 0,0,0,ParameterMode.ABSOLUTE,0)  
        # Signal production is done
        programObject.append_instruction(Command.CMD_SET_CONDITION, CommandMode.WCD, CND_PRODUCT_DONE,1,0,ParameterMode.ABSOLUTE,0)
        # Open the gripper
        programObject.append_instruction(Command.CMD_SET_CONDITION, CommandMode.WCD, CND_GRIPPER_OPEN,1,0,ParameterMode.ABSOLUTE,0)
        # Wait for 4000 ms
        programObject.append_instruction(Command.CMD_WAIT_TIME, CommandMode.WCD, 0,0,0,ParameterMode.ABSOLUTE,4000)
        # Close the gripper
        programObject.append_instruction(Command.CMD_SET_CONDITION, CommandMode.WCD, CND_GRIPPER_OPEN,0,0,ParameterMode.ABSOLUTE,0)
        # Wait for 500 ms
        programObject.append_instruction(Command.CMD_WAIT_TIME, CommandMode.WCD, 0,0,0,ParameterMode.ABSOLUTE,500)
        # Check End of the program?
        programObject.append_instruction(Command.CMD_JMPN, CommandMode.WCD, INDEX_MOVE_WAIT ,CND_FINISH,0,ParameterMode.ABSOLUTE,0)
        # ===========================================================================================================================
    ...
    if demonstrator_program.connect():
        ...
        demonstrator_program = generateProgram(demonstrator_program)
        demonstrator_program.execute()
